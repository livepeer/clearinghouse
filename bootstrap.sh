...
#   2. openmeter-collector/provision/bootstrap.sh -> catalog + billing profiles
...
#
# Billing (Path A): catalog also ensures the free/sandbox + Stripe PAYG billing
# profiles and writes openmeter-billing.json; those ids are merged into
# sdk-config.json.openmeter.
#
# Requires: jq. Auth0 provisioning also needs the auth0 CLI and an active
#           `auth0 login` session; catalog provisioning needs kongctl (+ curl
#           when installing the Stripe app via API key).
#
# Usage:
...
#   OPENMETER_STRIPE_API_KEY / STRIPE_SECRET_KEY
#                                 install Konnect Stripe app when missing
...
if [ "$SKIP_CATALOG" = 1 ]; then
  ...
  info "== OpenMeter catalog + billing =="
  OPENMETER_BILLING_OUT="$OUT_DIR/openmeter-billing.json" \
    OPENMETER_URL="$OPENMETER_URL" \
    bash "$CATALOG_PROVISIONER" catalog
fi

BILLING_JSON="$OUT_DIR/openmeter-billing.json"
...
billing_get() {
  ...
  if [ -f "$BILLING_JSON" ]; then
    jq -r --arg k "$key" '.openmeter[$k] // empty' "$BILLING_JSON"
  fi
}
...
SANDBOX_APP_ID="$(billing_get sandboxAppId)"
STRIPE_APP_ID="$(billing_get stripeAppId)"
FREE_PROFILE_ID="$(billing_get freeBillingProfileId)"
STRIPE_PROFILE_ID="$(billing_get stripeBillingProfileId)"
...
[ -f "$BILLING_JSON" ] && info "billing ids from $BILLING_JSON"
info ""
...
info "      Path A PAYG: copy openmeter.freeBillingProfileId / stripeBillingProfileId"
info "      into builder-api env (OPENMETER_FREE_BILLING_PROFILE_ID / OPENMETER_STRIPE_BILLING_PROFILE_ID)"
