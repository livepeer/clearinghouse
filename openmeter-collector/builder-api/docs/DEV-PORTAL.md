...
Operator runbook for publishing the builder-api **user usage** surface on Kong
...
validates Auth0-issued signer JWTs at the edge.

Audience API identifier: **`livepeer-clearinghouse`**.

Usage identity on builder-api is **user-scoped**:
`GET /api/v1/users/me/usage` and `GET /api/v1/users/me/balance` derive
`clientId` + `externalUserId` from the Bearer **signer JWT** (via
identity-webhook). There is no `clientId` path segment.
...
| `https://builder-api-production-82bf.up.railway.app` | Railway **builder-api** | Usage, balance, user upsert, RFC 8693 signer session |
| `https://pymthouse.us.auth0.com` | Auth0 tenant | `POST /oauth/token` signer mint (`sign:mint_user_token`) |
| `$KONNECT_PROXY_URL` | Kong Gateway (optional Phase C) | Edge OIDC for Auth0 Bearer / DCR only |
...
| Business rules | builder-api | Signer-JWT actor isolation, catalog groupBy |
...
| --- | --- | --- |
| Auth0 signer JWT (`Bearer`) | Yes (OIDC, audience `livepeer-clearinghouse`) | Yes (`GET /users/me/usage`, `/users/me/balance`) |
| Auth0 user JWT / DCR `client_credentials` | Yes at the edge if the strategy accepts it | **No** on usage — exchange first via `POST /api/v1/oidc/token` |
| `sk_*` | **No** (do not add Key Auth in v1) | Exchange only (`POST /api/v1/oidc/token`), not usage |
...
After Phase C, put `$KONNECT_PROXY_URL` first for JWT Bearer; keep Railway
documented for token exchange / `sk_*`.
...
### Signer mint (Auth0) vs token exchange (builder-api)
...
# 2) RFC 8693 exchange — Railway builder-api (user JWT or sk_* → signer JWT)
export BUILDER_API=https://builder-api-production-82bf.up.railway.app
# POST $BUILDER_API/api/v1/oidc/token
```
...
With the strategy linked to a Gateway Service, Konnect validates Auth0
Bearer tokens at the edge. builder-api then scopes rows to the signer JWT
actor (`app_client_id` + `external_user_id`).

| Token | Who | Usage result |
| --- | --- | --- |
| Signer JWT (`bearer`) | End user after mint or `POST /oidc/token` | Actor-scoped rows (intended self-serve path) |
| Portal DCR app `client_credentials` | App-level `azp` | Passes Gateway if OIDC accepts it; builder-api usage still requires a signer JWT |
...
2. Create an API from builder-api OpenAPI (`GET /api/v1/openapi.json`), scoped
   to `GET /api/v1/users/me/usage`, `GET /api/v1/users/me/balance` (and optional `/health` only).
...
2. **Routes** (usage + balance only):
   - `/api/v1/users/me/usage`
   - `/api/v1/users/me/balance`
   - Optional: `/health` only
   - **Do not** expose `/authorize`, `POST /api/v1/oidc/token`, or signer bus routes
...
4. Move OpenAPI `servers` / playground to `$KONNECT_PROXY_URL` (Railway stays
   as a secondary server for token exchange / `sk_*`).
...
enforces signer-JWT actor isolation even after a valid edge token.
...
2. Signer JWT via `$KONNECT_PROXY_URL` returns actor-scoped OpenMeter rows.
3. Unauthenticated calls fail at Gateway.
4. `sk_*` exchanges on Railway; unsupported through Gateway (documented).
...
- [x] OpenAPI `servers` default to local / Railway builder-api (pre-gateway)
